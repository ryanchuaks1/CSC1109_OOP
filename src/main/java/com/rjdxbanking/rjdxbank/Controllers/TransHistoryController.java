...
public class TransHistoryController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        interestRateLabel.setText("S$"
                + String.format("%.2f", SessionClient.getAccount().getYearlyProjectedInterestRate()));
        intializeTable();
    }
    ...
    void intializeTable() {
        ...
        transHistoryTable.getColumns().addAll(transTimeCol, transactionTypeCol, currencyCodeCol, transactionAmountCol,
                statusCol, toCol, fromCol);
        transHistoryTable.getSortOrder().add(transTimeCol);
        transHistoryTable.sort(); //XAVIER WAS HERE
    }
